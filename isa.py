...
class IsaAI:
    def __init__(self, use_pipeline=True):
        ...
        print(f"Initializing BLOOM-1b1 model. This may take a moment...")
        
        if use_pipeline:
            self.pipe = pipeline("text-generation", model=model_name, device_map="auto")
            print("Using BLOOM-1b1 pipeline for text generation.")
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(model_name)
            self.model = AutoModelForCausalLM.from_pretrained(model_name, device_map="auto")
            print("Using direct model loading for BLOOM-1b1.")
        
        self.use_pipeline = use_pipeline
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"Using device: {self.device}")
        
        print("Isa AI with BLOOM-1b1 model is ready!")
    ...
    def chat(self):
        self.speak("Hello, I am Isa, powered by the BLOOM-1b1 model. How can I assist you today?")
